...
getTrendings().then((response) => showTrendings(response));
...
showTrendings = (trendings) => {
  trendings.forEach((trending) => {
    ...
    gifsTrendContainer.insertBefore(
      gifTrendContainer,
      gifsTrendContainer.childNodes[2]
    );
...
function gifsOptions(gif, username, gifTitle, gifID, gifURL, gifContainer) {
  ...
  if (screenSize900.matches) {
    ...
    gif.addEventListener('mouseenter', async () => {
      ...
      const addFavsBtn = document.createElement('span');
      const removeFavBtn = document.createElement('span');
      const addDownloadBtn = document.createElement('span');
      const addmaxBtn = document.createElement('span');
...
function addFavGif(favBtn, removeFavBtn, gifInfo) {
  favBtn.addEventListener('click', async () => {
    ...
    if (document.body.classList.contains('favs')) {
      location.reload();
    }
  });
}
...
function expand(addmaxBtn, gifInfo) {
  addmaxBtn.addEventListener('click', async () => {
    ...
    if (
      addmaxBtn.parentNode.parentNode.parentNode.classList.contains(
        'mygifsContainer'
      )
    ) {
